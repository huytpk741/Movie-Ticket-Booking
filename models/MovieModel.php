...
class MovieModel extends Model
{
    ...
    public function add($created_by)
    {
        $name = $_POST["name"];
        ...
        // Save movie
        $sql = "INSERT INTO `movies`(`name`, `description`, `writer`, `director`, `duration`, `release_date`, `languages`, `price_per_ticket`, `created_by`) VALUES ('" . $name . "','" . $description . "','" . $writer . "','" . $director . "','" . $duration . "','" . $release_date . "','" . $languages . "','" . $price_per_ticket . "', '" . $created_by . "')";
    ...
    public function has_coupon_code($movie_id)
    {
        $sql = "SELECT * FROM coupon_codes WHERE movie_id = '" . $movie_id . "' AND valid_till > NOW()";
        $result = mysqli_query($this->connection, $sql);

        return mysqli_num_rows($result) > 0;
    }

    public function verify_coupon_code($movie_id, $coupon_code)
    {
        $sql = "SELECT * FROM coupon_codes WHERE movie_id = '" . $movie_id . "' AND valid_till > NOW() AND coupon_code = '" . $coupon_code . "'";
        $result = mysqli_query($this->connection, $sql);

        return mysqli_fetch_object($result);
    }

    public function get_coupon_code($movie_id)
    {
        $sql = "SELECT * FROM coupon_codes WHERE movie_id = '" . $movie_id . "' AND valid_till > NOW()";
        $result = mysqli_query($this->connection, $sql);
    ...
    public function do_delete($movie_id)
    {
        ...
        $sql = "DELETE FROM movie_ratings WHERE movie_id = '" . $movie_id . "'";
        mysqli_query($this->connection, $sql);

        $sql = "DELETE FROM comments WHERE movie_id = '" . $movie_id . "'";
        mysqli_query($this->connection, $sql);
    ...
    public function add_comment()
    {
        ...
        $sql = "INSERT INTO `comments`(`name`, `email`, `phone`, `message`, `movie_id`) VALUES ('" . $name . "', '" . $email . "', '" . $phone . "', '" . $message . "', '" . $movie_id . "')";
        mysqli_query($this->connection, $sql);
